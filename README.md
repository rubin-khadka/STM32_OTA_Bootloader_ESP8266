# STM32 OTA Bootloader with ESP8266 WiFi
...
[![ESP8266](https://img.shields.io/badge/ESP8266-WiFi-yellow)](https://www.espressif.com/en/products/socs/esp8266)
[![CRC32](https://img.shields.io/badge/CRC-32-darkgreen)](https://en.wikipedia.org/wiki/Cyclic_redundancy_check)

## Project Overview

This project implements an Over-The-Air (OTA) firmware update system for STM32F103C8T6 microcontrollers using ESP8266 for WiFi connectivity. The system allows firmware updates to be sent from a PC server to the embedded device over WiFi.
...
- When button is pressed: sends AT commands to ESP8266 via UART, connects to WiFi, connects to PC server via TCP, downloads firmware, stores it in W25Q64 external flash, sets OTA flag, and resets

**PC Server** – Python script that:
- Listens on port 5678
- Sends complete firmware image (16-byte header + binary) when requested

**ESP8266** – Runs AT firmware and acts as a WiFi modem:
- Receives AT commands from STM32
- Connects to WiFi and establishes TCP connection to PC server
- Forwards incoming firmware data to STM32
...
| ESP8266 (WiFi Module) | PA9 (TX), PA10 (RX) | UART1 - AT commands |
| W25Q64 (External Flash) | PB13 (SCK), PB14 (MISO), PB15 (MOSI), PB12 (CS) | SPI2 |
| USART2 (Debug) | PA2 (TX), PA3 (RX) | 115200 baud, 8N1 |

> **Note:** 
> - UART1 (PA9/PA10) communicates with ESP8266
> - UART2 (PA2/PA3) provides debug output
> - Multiple I2C devices (LCD, MPU6050, DS3231) share the same I2C bus (PB10/PB11) with different addresses
